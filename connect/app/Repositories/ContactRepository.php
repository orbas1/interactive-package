...
class ContactRepository
{
    ...
    {
        $sort_by      = $this->contact->getSortBy();
        $order        = $this->contact->getOrder('asc');
        $per_page     = $this->contact->getPerPage();
        $current_page = $this->contact->getCurrentPage();
    ...
    public function updateSegment(Request $request) : void
    {
        $contacts = Contact::whereIn('uuid', $request->uuids ?? [])->get();

        $segments = collect($request->segments ?? [])->pluck('uuid')->all();

        $segmentIds = Segment::whereIn('uuid', $segments)->pluck('id')->all();

        if ($request->action == 'add') {
            $contacts->each(function($contact) use ($request, $segmentIds) {
