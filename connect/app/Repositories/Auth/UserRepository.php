...
class UserRepository
{
    ...
    {
        $sort_by      = $this->user->getSortBy();
        $order        = $this->user->getOrder();
        $per_page     = $this->user->getPerPage();
        $current_page = $this->user->getCurrentPage();
        ...
        $query = $this->user->filterByName($name)->filterByUsername($username)->filterByEmail($email)->dateBetween(array(
    ...
    private function formatParams($uuid = null) : array
    {
        ...
        if ($gender && ! in_array($gender, ArrHelper::getList('genders'))) {
    ...
    /**
     * Update given user premiumship
     * @param User $user
     */
    public function premium(User $user) : User
    {
        $meta = $user->meta;
        $meta['is_premium'] = Arr::get($meta, 'is_premium') ? false : true;
        $user->meta = $meta;
        $user->save();
